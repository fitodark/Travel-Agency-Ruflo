-- =============================================================================
-- 0068 · `core.buscar_salidas` expone hora de llegada y la unidad (paso 2).
--
-- QUÉ RESUELVE. El prototipo del cliente (paso 2, "Horarios disponibles")
-- muestra por corrida: hora de salida → hora de LLEGADA, y el tipo de unidad
-- ("Suburban 18 plazas" / número económico). La búsqueda solo devolvía la hora
-- de salida del origen; la de llegada al destino y el tipo de unidad ya
-- existían en la base (`salida_parada.hora_paso_programada` de la parada
-- destino, y `core.salida.tipo_unidad_id` / `unidad_id`) pero nunca salían de
-- `core.buscar_salidas`. No hay ningún dato "de corrida" (folio/código) en el
-- esquema — el número económico de la unidad (`core.unidad.numero_economico`,
-- ya impreso en el ticket) es el identificador real más cercano; es NULLABLE
-- porque `unidad_id` es dato operativo que puede no estar asignado todavía.
--
-- DROP + CREATE por el `RETURNS TABLE` (patrón 0043/0051/0067).
-- =============================================================================
...
RETURNS TABLE (
  ...
  hora_llegada_destino timestamptz,
...
COMMENT ON FUNCTION core.buscar_salidas(date, uuid, uuid, integer, uuid, boolean, timestamptz) IS
  'Paso 2 del flujo de venta: salidas del día origen→destino (origen/destino = core.punto_ruta.id) con ruta, escalas, disponibilidad, tarifa general (columna `importe`), el mapa `tarifas` {categoria: importe}, el `mapa` de asientos congelado (paso 3) y el tipo/número económico de unidad + hora de llegada (paso 2). El origen debe permitir ascenso. Blueprint F4 · 05 §4.';
